...
const SKIP_ON_CHAIN =
  optionalEnv("WITHDRAW_SKIP_ON_CHAIN", "").toLowerCase() === "true";
...
async function submitWithdrawOrSkip(
  ...
  if (response.status === "failed") {
    ...
    console.log(
      `Skipping — UserOp returned failed status${
...
describe("Withdraw Funds Tests", () => {
  ...
  describe("withdraw: basic ETH happy path", () => {
    test(
      "submits an ETH withdrawal and confirms receipt",
      async () => {
        ...
        const response = await submitWithdrawOrSkip(
          client,
          wallet.address,
          req
        );
  ...
  describe("withdraw: ERC-20", () => {
    test(
      "submits a USDC withdrawal when the token is configured",
      async () => {
        const wallet = await tryFundedWallet(client);
        if (!wallet) {
          console.log("Skipping — WITHDRAW_SKIP_ON_CHAIN=true");
  ...
  describe("withdraw: edge cases (validation-only, no funding required)", () => {
    ...
    test(
      "accepts an alternate recipient address",
      async () => {
        const wallet = await tryFundedWallet(client);
        if (!wallet) {
          console.log("Skipping — WITHDRAW_SKIP_ON_CHAIN=true");
  ...
  describe("withdraw: response shape", () => {
    test(
      "returns the documented v4.WithdrawResponse shape",
      async () => {
        const wallet = await tryFundedWallet(client);
        if (!wallet) {
          console.log("Skipping — WITHDRAW_SKIP_ON_CHAIN=true");
  ...
  describe("paymaster reimbursement check", () => {
    test(
      "internal txs all succeed on paymaster-sponsored ETH withdrawal",
      async () => {
        const wallet = await tryFundedWallet(client);
        if (!wallet) {
          console.log("Skipping — WITHDRAW_SKIP_ON_CHAIN=true");

...
function fromCatalog(symbol: string): TokenDef {
  ...
  if (!entry) {
    throw new Error(
      `Expected ${symbol} in @avaprotocol/protocols Sepolia catalog`
    );
  }
  return {
...
describeOrSkip("getToken Tests", () => {
  ...
  test.each([
    ...
    "retrieve(%s) returns a well-formed found/not-found response",
    async (context, address) => {
  ...
  test.each([
    ...
    "retrieve(%s) rejects malformed addresses with HTTP 400 or 404",
  ...
  testOrSkip("handles concurrent requests properly", async () => {
    ...
    const responses = await Promise.all(
      addresses.map((a) => client.tokens.retrieve(a))
    );
  ...
  testOrSkip.each(
    Object.entries(TOKENS).map(
      ([name, token]) => [name, token, token.decimals] as const
    )
  )(
  ...
  test("request completes within a reasonable time budget", async () => {
    ...
    const resp = await client.tokens.retrieve(

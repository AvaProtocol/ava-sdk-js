...
describe("secret Tests", () => {
  ...
  describe("create secret suite", () => {
    ...
    it("secrets from different eoaAddress do not cross", async () => {
      ...
      expect(createResultClient1.success).toBeTruthy();
      expect(createResultClient2.success).toBeTruthy();
      ...
      expect(
        listResultClient1.items.some((item) => item.name === inputName2)
      ).toBe(false);

      expect(
        listResultClient2.items.some((item) => item.name === inputName1)
      ).toBe(false);
    });
  });
  ...
  describe("delete secret suite", () => {
    ...
    it("delete at workflowId level succeeds", async () => {
      ...
      const workflowResult = await client.createSecret(
        workflowLevelName,
        "value2",
        {
          workflowId: inputWorkflowId,
      ...
      expect(userDeleted.success).toBeTruthy();
  ...
  describe("pagination suite", () => {
    ...
    it("should filter secrets by workflowId", async () => {
      ...
      expect(typeof filteredSecrets.pageInfo.hasNextPage).toBe("boolean");
      expect(typeof filteredSecrets.pageInfo.hasPreviousPage).toBe("boolean");

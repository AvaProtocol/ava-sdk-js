...
describe("Input Field Tests", () => {
  ...
  test("should reject workflow with invalid node name", async () => {
    ...
    const workflowProps = {
      ...
      inputVariables: { settings: getSettings(wallet.address, "TestInvalidWorkflow") },
    };
  ...
  test("should show input data for both trigger and node in execution steps using comprehensive manual trigger config", async () => {
    ...
    try {
      ...
      const workflowProps = {
        ...
        expiredAt: getExpiredAt("24h"),
        smartWalletAddress: wallet.address,
        inputVariables: { settings: getSettings(wallet.address, "TestWorkflowWithManualTriggerInput") },
      };
      ...
      // Test the trigger config - should always be populated in execution steps
    ...
    const customCodeNode = NodeFactory.create({
      ...
      data: {
        ...
        source: `
            ...
            const chainId = eventData.chainId || 11155111; // Default to Sepolia for simulation

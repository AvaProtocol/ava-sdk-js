...
- After the wallet was funded, `withdraw.test.ts` failed with `eth_sendUserOperation: replacement underpriced`. CI matrix jobs (and sequential withdraws in the same file) share MA v2 salt-0. Contention retry now lives in `retryOnUserOpContention` / `triggerUserOpAndAssert` (withdraw, gasTracking, contractWrite trigger). Funded trigger workflows use `maxExecution: 10` so a failed send does not exhaust the slot. Verified withdraw locally against `http://localhost:8080` (11/11).

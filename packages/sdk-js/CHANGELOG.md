...
- e443b87: EOA 7702 delegation API (`wallets.prepareDelegation` / `submitDelegation` / `getDelegation` / `delegate`) and native ETH session-grant fields from AVS OpenAPI.
...
- Updated dependencies [e443b87]
  - @avaprotocol/types@4.8.0

## 4.14.0

### Minor Changes

- e75f9ea: Bump `@avaprotocol/protocols` to `^1.0.0`. Spreads `Chains.PolygonMainnet` (137) and `Chains.HyperliquidMainnet` (999) plus `wrapped.weth` → WPOL / WHYPE, so callers can resolve those wrappers from the catalog instead of inlining addresses. No Uniswap/AAVE on HyperEVM; Polygon protocol maps are a catalog follow-up. `Chains.Holesky` stays as a deprecated SDK alias (17000) so this remains a 4.x minor — use `Chains.Sepolia` for testnet work. Collapses the expansion-chain JWT `aud` probes in `auth.test.ts` onto a `test.each` table keyed by `Chains`. Adds Railway E2E for Polygon WPOL `contractRead` / `simulate`; Hyperliquid WHYPE `contractRead` only (Tenderly has no 999).

## 4.13.0

### Minor Changes

- 1252a83: Typed UserOp inner calls on execute receipts and a JWT-gated status GET (N14.a / EigenLayer-AVS #774).
...
- Updated dependencies [1252a83]
  - @avaprotocol/types@4.7.0

## 4.12.0

### Minor Changes

- a088a42: `wallets.list({ chainId })` — list a specific chain's smart wallets.
...
- Updated dependencies [a088a42]
  - @avaprotocol/types@4.6.0

## 4.11.0

### Minor Changes

- c088142: Bump `@avaprotocol/protocols` to `^0.12.0`. Unichain (130) and Robinhood (4663) chain IDs plus Uniswap V3 / WETH now resolve from the catalog, so `SessionPolicyActions.uniswapV3Swap(130)` and `uniswapV3Swap(4663)` no longer throw. Adds Railway E2E for expansion-chain auth + WETH `contractRead` / `simulate`.
...
- 3d5f8f2: feat: partner assertion minting for permission-gated gateway reads
...
- e7b783a: feat: session policies — the smart-wallet spend-grant surface
...
- Updated dependencies [e7b783a]
  - @avaprotocol/types@4.4.0

## 4.5.0

### Minor Changes

- a23a2ff: Surface the gateway's execution-budget fields on the `Workflow` response.
...
- Updated dependencies [a23a2ff]
  - @avaprotocol/types@4.3.0

## 4.4.0

### Minor Changes

- f873dd7: Support atomic approve+swap batching on a single `contractWrite` node.
...
- Updated dependencies [f873dd7]
  - @avaprotocol/types@4.2.0
...
- 4b958c9: Durable execution: add the `await` pause node and execution `signal` endpoint. A workflow can now pause mid-execution and resume later, exactly-once, surviving aggregator restarts. Adopts the EigenLayer-AVS durable-execution work (SDK hand-off, PRs #643–#647) and regenerates `openapi.gen.ts` from staging.
...
- Updated dependencies [4b958c9]
  - @avaprotocol/types@4.1.0

## 4.0.0

### Major Changes

- ba27712: Chain decoupling: `chainId` is now a property of each chain-aware part, not of the workflow. Adopts the EigenLayer-AVS renovation (PR #634) and regenerates `openapi.gen.ts` from staging.
...
- Updated dependencies [ba27712]
  - @avaprotocol/types@4.0.0

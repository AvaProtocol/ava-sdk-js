...
5. **Capture baseline timestamp**
   ...
   Run: `gh api repos/AvaProtocol/ava-sdk-js/pulls/<PR_NUMBER>/reviews --jq '[.[] | select(.user.login == "copilot-pull-request-reviewer[bot]")] | sort_by(.submitted_at) | last | .submitted_at'`

6. **Request Copilot Review**
   Run: `gh api repos/AvaProtocol/ava-sdk-js/pulls/<PR_NUMBER>/requested_reviewers -f 'reviewers[]=copilot-pull-request-reviewer[bot]' --method POST`

 ...
 * 5. Updated method names to match current API: getWorkflow, setWorkflowEnabled, deleteWorkflow, triggerWorkflow
 * 6. Support for querying any owner's wallets by passing owner address directly (requires AVS_API_KEY)
 *
 * ⚠️ Important Note on getWallet vs getWallets:
 * Wallet addresses are derived from: owner + factory + salt
 * - getWallet with salt "0" (without factory) uses the default factory from server config
 * - getWallets returns wallets that may have been created with different factory addresses
 * - If getWallets shows a wallet with salt "0" using factory X, you must pass factory X to getWallet
 *   to get the same address: getWallet({ salt: "0", factoryAddress: "0x..." })
 * - Always check getWallets first to see what factory address was used for existing wallets
 *
 * 🔑 Querying Any Owner's Wallets:
 * - getWallets [owner-address]: Query wallets for a specific owner address (requires AVS_API_KEY)
 *   Example: yarn start getWallets 0x1234...5678
 * - getWallet <owner-address> <salt> [factory-address]: Get/create wallet for a specific owner (requires AVS_API_KEY)
...
const main = async (cmd: string) => {
  ...
  // Handle --help flag
  if (commandArgs.command === "--help") {
    const helpCommand = commandArgs.args[0];
    if (helpCommand === "getWallet") {
      console.log(`
📖 getWallet Command Help

Get or create a smart wallet address for a given salt and optional factory address.

USAGE:
  For authenticated user:
    yarn start getWallet <salt> [factory-address]
  
  For specific owner (requires AVS_API_KEY):
    yarn start getWallet <owner-address> <salt> [factory-address]

EXAMPLES:

  # Get wallet with salt "0" for authenticated user
  yarn start getWallet 0
  
  # Get wallet with salt "1" and specific factory for authenticated user
  yarn start getWallet 1 0x1234567890123456789012345678901234567890
  
  # Get wallet for a specific owner (requires AVS_API_KEY)
  yarn start getWallet 0xc60e71bd0f2e6d8832Fea1a2d56091C48493C788 0
  
  # Get wallet for specific owner with factory address
  yarn start getWallet 0xc60e71bd0f2e6d8832Fea1a2d56091C48493C788 0 0x1234567890123456789012345678901234567890
  
  # On Base mainnet
  yarn start --avs-target base getWallet 0
  
  # On Base mainnet for specific owner
  yarn start --avs-target base getWallet 0xOwnerAddress 0

IMPORTANT NOTES:

  ⚠️  Wallet addresses are derived from: owner + factory + salt
  
  • If you don't specify a factory address, the server's default factory is used
  • If getWallets shows a wallet with a different factory, you must pass that factory
  • Always check getWallets first to see what factory address was used for existing wallets
  
  Example workflow:
    1. Check existing wallets: yarn start getWallets
    2. Note the factory address for salt "0" (if any)
    3. Use that factory: yarn start getWallet 0 0xFactoryAddress

AUTHENTICATION:

  • For your own wallets: Uses TEST_PRIVATE_KEY (signature-based auth)
  • For other owners: Requires AVS_API_KEY (admin access)
  • API key allows querying any owner's wallets, not just your own
...
`);
    ...
    // General help
    console.log(`
📖 AVA SDK Example Script Help

Available commands:
  getWallet, getWallets, getWorkflows, getWorkflow, getExecutions, getExecution
  schedule, schedule-cron, schedule-monitor, schedule-telegram, schedule-sweep
  disableWorkflow, deleteWorkflow, examineWorkflow, trigger
  create-secret, list-secrets, withdraw, hideAllWallets
  ...
  switch (commandArgs.command) {
    ...
    case "getWallet": {
      ...
      if (firstArg && firstArg.startsWith("0x") && firstArg.length === 42) {
        // First arg is owner address - requires API key
        ...
        if (!salt) {
          ...
          console.error("");
          console.error("USAGE:");
          console.error("  For authenticated user:");
    ...
    default:
      ...
      getWallet <salt> [factory-address]:                  to create/get a smart wallet with a salt for authenticated user
      getWallet <owner-address> <salt> [factory-address]: to create/get a smart wallet for a specific owner (requires AVS_API_KEY)
      getWallets [owner-address]:                        to list smart wallet addresses (pass owner address to query any owner, requires AVS_API_KEY)
